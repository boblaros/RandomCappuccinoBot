...
def register_user_handlers(bot: TeleBot, user_feedback, verification_codes):
    ...
    def feedback_callback(call):
        ...
        # Определяем, кто отвечает: user1 или user2
        ...
        if action == "yes":
            ...
            def collect_failure_reason(message):
                reason = message.text

                # Новое соединение для базы данных
                with sqlite3.connect(DB_PATH) as conn_failure_reason:
                    cursor_conn_failure_reason = conn_failure_reason.cursor()
                    cursor_conn_failure_reason.execute("SELECT failure_reason FROM pair_registry WHERE pair_id = ?",
                                                       (pair_id,))
                    current_reason = cursor_conn_failure_reason.fetchone()[0]

                    # Обновление общего поля failure_reason
