...
from code.config import SMTP_SERVER, SMTP_PORT, EMAIL_ADDRESS, EMAIL_PASSWORD, ADMIN_IDS
from code.utils.db import (get_users_from_db,
...
def send_user_details(bot, chat_id, users):
        ...
        for user in users:
            ...
            bot.send_message(chat_id, user_details, parse_mode="Markdown")
...
def send_broadcast_message(bot, message):
        ...
        for user in users_list:
            ...
            try:
                ...
                fail_count += 1
                print(
                    f"Неизвестная ошибка при отправке сообщения пользователю {user_id if user_id else 'unknown'}: {e}")
